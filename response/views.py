...
def view_class_weekday(grade, division, dt):
    assert (grade, division) in SHINYANG[this_year][
        this_semester]["GRADE_DIVISION"]
    wd = weekday(dt)
    ...
    for i in range(periods):
        try:
            ...
            rows.append("{}교시 {} {}-{}".format(i + 1,
                                               row.subject, row.grade, row.division))
        except:
            rows.append("{}교시 -".format(i + 1))
    message = '\n'.join(rows)
            ...
            try:
                ...
                # option exists
                    ...
                    if option == "지금":
                        if len(target.split("-")) > 1:
                            q_option = {
                                'grade_division': target, 'option': option}
                            text = view_class(
                                target, {"now": True, "datetime": now})
                        else:
                            # teacher now
